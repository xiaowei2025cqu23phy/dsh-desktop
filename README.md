...
- **手机 PWA 遥控**:扫码配对(自动填地址+令牌),手机上发任务、看流式进展、停止任务;**审批/提问卡片一键应答**——agent 卡住等你批准时不再"失联"。远程访问仅面向可信局域网,默认启用 2 小时自动关闭;**禁止使用内网穿透、端口转发或公网反向代理暴露 Harness**。控制权在桌面端:可一键暂停全部连接、对单个设备暂停/拉黑(被拉黑设备令牌正确也拒绝)、锁屏/睡眠自动暂停,并支持只监听当前局域网 IP(默认 0.0.0.0 全网卡,含 VPN/虚拟网卡)。
- **QQ / Telegram 机器人通道**:私聊发指令即干活(群聊只聊天、不响应任何指令);打通**主动推送**(QQ 交互后 48h 窗口),任务完成、失败、要审批都会主动找你;低风险审批带「允许/拒绝」内联按钮,点一下即批(高风险操作转桌面端确认);支持**白名单门禁**(留空 = 锁定)与**扫码登录**自动获取机器人凭据。
...
「设置 → QQ 机器人」填入在 [QQ 开放平台](https://q.qq.com) 注册机器人得到的 AppID/AppSecret 即启用(留空自动禁用),**并填「允许的用户 openid」白名单(留空 = 锁定,机器人不服务任何聊天;与 Telegram 同语义)**,也可设置**默认工作区/目录**(任务命令未指定时自动使用)。在 QQ 私聊机器人发送指令(发送任意无法识别的消息,机器人会自动回复完整指令集与示例):
