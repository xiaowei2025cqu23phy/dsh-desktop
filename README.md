...
- **手机 PWA 遥控**:扫码配对(自动填地址+令牌),手机上发任务、看流式进展、停止任务;**审批/提问卡片一键应答**——agent 卡住等你批准时不再"失联"。
...
- **工作区**:列表、新建、按工作区执行任务
- **安全**:Bearer token 认证 + RPC 白名单(手机端不能改配置/读密钥/操作文件系统),仅局域网可达
...
**模式提示词**:`任务 xxx` 等指令下,agent 以**专业助手**身份工作;对话模式下以**朋友**口吻聊天;两套提示词可在桌面端「设置 → QQ 机器人」自定义(留空 = 不注入)。
